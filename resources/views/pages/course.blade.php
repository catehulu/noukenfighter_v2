@extends('layouts.app')

@section('content')

    <h1>Course</h1>
    <h3>Welcome to our Course page where you can review some of the words you have learned throughout your journey learning in our website. Each word has a corresponding picture and the details on how to memorize them properly.</h3>
    <br>

    <div class="card border-primary">
      <img class="card-img-top" src="holder.js/100px180/" alt="">
      <div class="card-body">
        <h4 class="card-title">Kana Course</h4>
        <div class="row">
            <div class="col-sm-6">
                <form action="/course/kana/katakana">
                <input type="submit" value="Katakana" class="btn btn-lg" style="background-color: #ffccff">
                </form>
            </div>
            <div class="col-sm-6">
                    <form action="/course/kana/hiragana">
                <input type="submit" value="Hiragana" class="btn btn-lg" style="background-color: #ffccff">
                </form>
            </div>
        </div>
      </div>
    </div>

    <div class="card border-primary">
      <div class="card-body">
        <h4 class="card-title">Kanji Course</h4>
            <div class="row">
                <div class="col-sm-2">
                    <form action="/course/kanji/N5">
                    <input type="submit" value="N5" class="btn btn-lg" style="background-color: #d99efb">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kanji/N4">
                    <input type="submit" value="N4" class="btn btn-lg" style="background-color: #90e1f9">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kanji/N3">
                    <input type="submit" value="N3" class="btn btn-lg" style="background-color: #a5f69d">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kanji/N2">
                    <input type="submit" value="N2" class="btn btn-lg" style="background-color: #e7f695">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kanji/N1">
                    <input type="submit" value="N1" class="btn btn-lg" style="background-color: #fd8c8c">
                    </form>
                </div>
            </div>
      </div>
    </div>

    

    <div class="card border-primary">
      <div class="card-body">
        <h4 class="card-title">Vocabulary Course</h4>
            <div class="row">
                <div class="col-sm-2">
                    <form action="/course/kosakata/N5">
                    <input type="submit" value="N5" class="btn btn-lg" style="background-color: #d99efb">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kosakata/N4">
                    <input type="submit" value="N4" class="btn btn-lg" style="background-color: #90e1f9">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kosakata/N3">
                    <input type="submit" value="N3" class="btn btn-lg" style="background-color: #a5f69d">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kosakata/N2">
                    <input type="submit" value="N2" class="btn btn-lg" style="background-color: #e7f695">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/kosakata/N1">
                    <input type="submit" value="N1" class="btn btn-lg" style="background-color: #fd8c8c">
                    </form>
                </div>
            </div>
      </div>
    </div>

    

    <div class="card border-primary">
      <div class="card-body">
        <h4 class="card-title">Grammar Course</h4>
            <div class="row">
                <div class="col-sm-2">
                    <form action="/course/grammar/N5">
                    <input type="submit" value="N5" class="btn btn-lg" style="background-color: #d99efb">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/grammar/N4">
                    <input type="submit" value="N4" class="btn btn-lg" style="background-color: #90e1f9">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/grammar/N3">
                    <input type="submit" value="N3" class="btn btn-lg" style="background-color: #a5f69d">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/grammar/N2">
                    <input type="submit" value="N2" class="btn btn-lg" style="background-color: #e7f695">
                    </form>
                </div>
                <div class="col-sm-2">
                    <form action="/course/grammar/N1">
                    <input type="submit" value="N1" class="btn btn-lg" style="background-color: #fd8c8c">
                    </form>
                </div>
            </div>
      </div>
    </div>
@endsection
